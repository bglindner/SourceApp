...
def read_map(args):
    ...
    database = args['sourceapp_database'] + '/database.fna'
    print("Running read mapping...")
    print("This can take a while.")
    try:
        subprocess.run(["strobealign --use-index " + database + " " + fwd + " " + rev + " -t " + str(threads) + " | samtools sort "+ samthreads + " -o " + output + " -"], shell=True, check=True)
    except Exception as e:
