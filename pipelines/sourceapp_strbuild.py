...
def genome_derep(args):
    ...
    if noderep:
        ...
        new_sdf.to_csv(output_dir + "/sources.txt",sep="\t",index=False,mode="w")
...
def build_database(args):
    ...
    try:
      subprocess.run(["strobealign --create-index -t " + threads + " " + output_dir + "/database.fna -r " + length],shell=True,check=True,stderr=subprocess.DEVNULL)
    except Exception as e:
    ...
    # clean up
    #subprocess.run(["rm " + output_dir + "/final_genome_list.txt " + output_dir + "/sinfo.csv " + output_dir + "/*glist.txt " 
    #                + output_dir + "/contigs.txt " + output_dir + "/*.ginfo.csv"], shell=True,check=True)
    #subprocess.run(["rm -r " + output_dir + "/drep* " + output_dir + "/checkm2],shell=True,check=True)
...
def flag_crx(workdir):
    ...
    for x in range(len(cdf["genome"])):
        ...
        if len(ind) == 1:
            ...
            for y in range(len(cdf.loc[ind]["genome"])):
                name = cdf.loc[ind]["genome"].iloc[y]
                slst.append(sdf[sdf["genome"]==name]["source"].iloc[0])
            if len(set(slst)) > 1:
...
def main():
    ...
    parser.add_argument(
        '-t', '--threads',
        help="Threads available to SourceApp build",
        metavar="",
        type=int,
        default=1,
        required=False
    ...
    parser.add_argument(
        '-q', '--genome-quality',
        help="Aggregate quality score threshold for accepting input genomes (float, 50 default)",
        metavar="",
        type=float,
        default=50,
        required=False
    ...
    parser.add_argument(
        ...
        help="Disable genome dereplication. This will create the database using all \
            of the provided genomes passing quality requirements.",
        action="store_true",
        required=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '-r', '--read-length',
        help="Specify the read length of the metagenomes you intend to use with this SourceApp database.",
        metavar="",
        type=float,
        default=150,
        required=False
    ...
    print("Step 04: indexing database for strobealign")
    build_database(args)
